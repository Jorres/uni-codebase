...
def scatter_dots(dots, ys, xmax, ymax, xmin, ymin):
    pxs = []
    pys = []
    nxs = []
    nys = []
    for (dot, y) in zip(dots, ys):
        if int(y) > 0:
            pys.append((float(dot[0]) - 1.2 * xmin) / xmax * GRID_SIZE)
            pxs.append((float(dot[1]) - 1.2 * ymin) / ymax * GRID_SIZE)
        else:
            nys.append((float(dot[0]) - 1.2 * xmin) / xmax * GRID_SIZE)
            nxs.append((float(dot[1]) - 1.2 * ymin) / ymax * GRID_SIZE)
...
def visualize(ansamble, bs, num, train_dots, test_dots, train_ys, test_ys):
    raw_xs = [float(d[0]) for d in train_dots]
    ...
    scatter_dots(train_dots, train_ys, xmax, ymax, xmin, ymin)
    scatter_dots(test_dots, test_ys, xmax, ymax, xmin, ymin)
...
def train_adaboost(data, ys, test_data, test_ys, boost_size):
    ...
    for i in range(boost_size):
        weak = tree.DecisionTreeClassifier(splitter = "random", max_depth = WEAK_DEPTH)
