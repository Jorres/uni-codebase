...
def predict_naive_regression(dataset, target_pos, window, kernel_function, distance_function):
    ...
    return round(nomin / denom if denom != 0 else sum(labels) / n)
...
def predict_progressive_regression(dataset, target_pos, k, kernel_function, distance_function):
    ...
    for i in range(3):
        ...
        prediction = nomin / denom if denom != 0 else sum(labels) / n
...
def draw_window_dependency(dataset, datalen, window, kernel, distance_function):
    ...
    for i in range(20):
        window = i + 2
        k = window
        f_score = do_progressive_regression(dataset, datalen, window, kernel, distance_function)[0]
        x.append(window)
        y.append(f_score)
        
    plt.xticks([i for i in range(20)])

    plt.xlabel("$knn, triangular, euclidean$")
    plt.ylabel("$F-score$")
    plt.plot(x, y, '-o')
    plt.show()
