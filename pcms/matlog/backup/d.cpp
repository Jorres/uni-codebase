...
bool evaluate_node(node* v, const vector<node*>& assumptions) {
    if (v->is_op) {
        if (v->op == '!') {
            v->value = static_cast<bool>(!evaluate_node(v->l, assumptions));
        } else if (v->op == IMPLICATION) {
            v->value = static_cast<bool>(!evaluate_node(v->l, assumptions) || evaluate_node(v->r, assumptions));
        } else if (v->op == '&') {
            v->value = static_cast<bool>(evaluate_node(v->l, assumptions) && evaluate_node(v->r, assumptions));
        } else if (v->op == '|') {
            v->value = static_cast<bool>(evaluate_node(v->l, assumptions) || evaluate_node(v->r, assumptions));
...
public:
    ...
    void prove(vector<int>& selection) {
        ...
        for (auto& data : prefixes) {
            ...
            cur++;
            cerr << "out lines after " << cur << " : " << out_lines << endl;
        }
    ...
    vector<parser> deduction_transfer(vector<parser>& parsers, node* left, const vector<node*>& assumptions, bool should_write) {
        ...
        for (auto& p : parsers) {
            node* right = p.top();
            node* whole = make_impl(left, right);
    ...
    void generate_and_false(node* v, vector<parser>& parsers, const vector<node*>& selection) {
        ...
        parser p1 = chop_first_impl(p0);
        parser p2(wrap_in_ax_5(v, true_left));
...
void test() {
    ...
    p.generate_or_false(p.top(), doc, assumptions);
...
void print_axioms() {
    ...
    parser b("B");
    parser c("C");
    print_node(wrap_in_ax_1(a.top(), b.top()));
    print_node(wrap_in_ax_2(a.top(), b.top(), c.top()));
    print_node(wrap_in_ax_or(a.top(), b.top()));
    print_node(wrap_in_ax_and(a.top(), b.top()));
    print_node(wrap_in_ax_8(a.top(), b.top(), c.top()));
    print_node(wrap_in_ax_9(a.top(), b.top()));
    print_node(wrap_in_ax_10(a.top()));
}
